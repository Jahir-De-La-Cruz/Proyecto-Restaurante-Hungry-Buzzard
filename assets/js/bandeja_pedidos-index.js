document.addEventListener('DOMContentLoaded', function () {
    ...
    cartButtonBuy.addEventListener('click', () => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                const url = `pags/formulario_compra.php?productos=${encodeURIComponent(productsInfo)}&precio=${encodeURIComponent(totalPrice.toFixed(2))}`;
    ...
    function buyIndividual(event) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                // Redirigir a formulario_compra.php con los datos en la URL
                const url = `pags/formulario_compra.php?productos=${encodeURIComponent(productTitle)}&precio=${encodeURIComponent(productPrice.toFixed(2))}`;
                window.location.href = url;
            }
        });
    }
